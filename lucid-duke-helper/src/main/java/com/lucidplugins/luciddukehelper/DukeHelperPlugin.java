...
import com.lucidplugins.luciddukehelper.api.util.*;
...
@Extension
@PluginDescriptor(name = "Lucid Duke Helper", description = "Auto-prayers and other helper features and overlays for Duke Succ", enabledByDefault = false, tags = {"succ", "duke", "lucid", "helper"})
public class DukeHelperPlugin extends Plugin
{
    ...
    private void onMenuOptionClicked(MenuOptionClicked event)
    {
        ...
        if (event.getMenuAction() == MenuAction.WIDGET_TARGET)
        {
            ...
            {
                event.consume();
                InteractionUtils.useItemOnNPC(InventoryUtils.getFirstItem("Arder powder"), getDukeSuc());
            }
            else if (config.oneClickDust() && event.getMenuTarget().contains("Musca powder") && InventoryUtils.count("Musca powder") > 0 && dukeSleeping())
            {
                event.consume();
                InteractionUtils.useItemOnNPC(InventoryUtils.getFirstItem("Musca powder"), getDukeSuc());
            }
        }
    }
    ...
    private void onNpcSpawned(NpcSpawned event)
    {
        ...
        if (event.getActor() instanceof NPC && event.getActor().getId() == VENT_NPC)
        {
    ...
    {
        return NpcUtils.getNearest(npc -> npc.getName() != null && npc.getName().contains("Duke Suc"));
    }
}
