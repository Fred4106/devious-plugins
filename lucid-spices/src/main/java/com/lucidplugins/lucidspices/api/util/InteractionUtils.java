...
{
    public static void useItemOnWallObject(Client client, Item item, TileObject tileObject)
    {
        MousePackets.queueClickPacket();
        final ItemComposition itemComposition = client.getItemComposition(item.getId());
        final ObjectComposition objectComposition = client.getObjectDefinition(tileObject.getId());
        ...
        client.invokeMenuAction("Use", "<col=ff9040>" + itemComposition.getName() + "</col><col=ffffff> -> <col=ffff>" + objectComposition.getName(), tileObject.getId(), MenuAction.WIDGET_TARGET_ON_GAME_OBJECT.getId(), tileObject.getLocalLocation().getSceneX(), tileObject.getLocalLocation().getSceneY(), -1, -1);
    }
